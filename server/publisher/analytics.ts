...
import type { PublisherPost } from "../../drizzle/schema";
...
export function normalizeSourcePerformanceFilters(input: { sourceName?: string; from?: string; to?: string } = {}) {
  const parseDate = (value: string | undefined, endOfDay: boolean) => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) throw new Error("Analytics dates must use the YYYY-MM-DD format.");
    const date = new Date(`${value}T${endOfDay ? "23:59:59.999" : "00:00:00.000"}Z`);
    if (Number.isNaN(date.getTime())) throw new Error("The analytics date is not valid.");
    return date;
  };
  const sourceName = input.sourceName?.trim() || undefined;
  const from = parseDate(input.from, false);
  const to = parseDate(input.to, true);
  if (from && to && from > to) throw new Error("The start date cannot be after the end date.");
...
export function normalizeAnalyticsPresetName(value: string) {
  ...
  if (name.length < 2 || name.length > 80) throw new Error("Preset name must be between 2 and 80 characters.");
  return name;
}
...
export function isValidTelegramRecipientCode(input: { code: string; codeHash: string | null; expiresAt: Date | null; now?: Date }) {
  return Boolean(input.codeHash && input.expiresAt && input.expiresAt > (input.now ?? new Date()) && input.codeHash === hashText(input.code.trim().toUpperCase()));
}
