...
function GeneralSettingsCard() {
  ...
  const [form, setForm] = useState<{
    ...
    postLanguage: "fa" | "en" | "de" | "custom";
  } | null>(null);
  useEffect(() => {
    if (settings && !form) {
      const language = POST_LANGUAGE_OPTIONS.some(
        option => option.value === settings.postLanguage
      )
        ? (settings.postLanguage as "fa" | "en" | "de" | "custom")
        : "fa";
      setForm({
        channelHandle: settings.channelHandle,
        appName: settings.appName ?? "",
        postSignature: settings.postSignature ?? "",
  ...
  const updateLanguage = (value: string) => {
    const language = POST_LANGUAGE_OPTIONS.some(
      ...
      ? (value as "fa" | "en" | "de" | "custom")
      : "fa";
    setForm(current =>
      current ? { ...current, postLanguage: language } : current
    );
  };
  return (
    <Card className="border-slate-200/80 shadow-sm">
      ...
      <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={event =>
                setForm(current =>
                  current
                    ? { ...current, channelHandle: event.target.value }
                    : current
                )
              }
              placeholder={t("settings.channelHandlePlaceholder")}
            />
            <p className="mt-1.5 text-xs leading-5 text-slate-500">
              {t("settings.channelHandleHint")}
            </p>
          </div>
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-800">
          ...
          <div>
            ...
            <Input
              ...
              onChange={event =>
                setForm(current =>
                  current
                    ? { ...current, postSignature: event.target.value }
                    : current
                )
              }
        ...
        <Button
          ...
          onClick={() =>
            save.mutate({
              channelHandle: form.channelHandle,
              appName: form.appName,
              postSignature: form.postSignature,
              postLanguage: form.postLanguage,
            })
          }
        >
...
function LlmSettingsCard() {
  ...
  const test = trpc.publisher.testLlm.useMutation({
    onSuccess: result =>
      toast.success(t("settings.testOk", { model: result.model })),
    onError: error => toast.error(error.message),
  });
  const modelsQuery = trpc.publisher.listModels.useQuery(undefined, {
    enabled: false,
  });
    ...
    <Card className="border-slate-200/80 shadow-sm">
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          ...
          <Badge
            className={
              status?.effective.apiKeyConfigured
                ? "bg-emerald-600"
                : "bg-amber-500"
            }
          >
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            placeholder={
              status?.apiKeyConfigured && status.apiKeyLast4
                ? t("settings.apiKeyConfiguredAs", {
            ...
            autoComplete="off"
          />
          <div className="mt-1.5 flex items-start gap-3 rounded-2xl bg-slate-50 p-3 text-xs leading-5 text-slate-600">
            <LockKeyhole className="mt-0.5 h-4 w-4 shrink-0 text-slate-500" />
            <p>{t("settings.apiKeyHint")}</p>
          </div>
        </div>
        {status ? (
          <p className="text-xs text-slate-500" dir={dir}>
            {t("settings.effectiveModel", { model: status.effective.model })} ·{" "}
            {status.effective.modelSource === "settings"
...
export default function PublisherSettings() {
  ...
  const refresh = () => {
    utils.publisher.dashboard.invalidate();
    utils.publisher.history.invalidate();
  };
  ...
  const createSource = trpc.publisher.createSource.useMutation({
    onSuccess: () => {
      toast.success(t("settings.toastSourceCreated"));
      setSourceForm(emptySource);
      refresh();
    },
    onError: error => toast.error(error.message),
  });
  const updateSource = trpc.publisher.updateSource.useMutation({
    onSuccess: () => {
      toast.success(t("settings.toastSourceUpdated"));
      setSourceForm(emptySource);
  ...
  const setActive = trpc.publisher.setSourceActive.useMutation({
    onSuccess: (_result, variables) => {
      toast.success(
        variables.active
          ? t("settings.toastSourceActivated")
          : t("settings.toastSourceDeactivated")
      );
      refresh();
    },
    onError: error => toast.error(error.message),
  });
  const savePreferences = trpc.publisher.updateEditorialPreferences.useMutation(
    {
      onSuccess: () => {
        toast.success(t("settings.toastEditorialSaved"));
        refresh();
      },
      onError: error => toast.error(error.message),
    }
  ...
  const submitSource = () => {
    const payload = {
      ...sourceForm,
      feedUrl: sourceForm.feedUrl.trim() || null,
    };
    if (editingSourceId)
      ...
      <div dir={dir} className="mx-auto max-w-6xl space-y-6 pb-10">
        ...
        <Card className="border-slate-200/80 shadow-sm">
          <CardHeader>
            <div className="flex items-start justify-between gap-4">
              ...
              <Badge
                className={tokenConfigured ? "bg-emerald-600" : "bg-amber-500"}
              >
                {tokenConfigured
                  ? t("settings.tokenConfigured")
                  : t("settings.tokenNeeded")}
          ...
          <CardContent className="space-y-4">
            ...
            <Button
              variant="outline"
              onClick={() => toast.info(t("settings.tokenGuideToast"))}
            >
              {t("settings.tokenGuideBtn")}
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card className="border-violet-200 bg-violet-50/40 shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                ...
                {t("settings.editorialTitle")}
              </CardTitle>
              <CardDescription>{t("settings.editorialDesc")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-800">
                ...
                <Textarea
                  ...
                  maxLength={2000}
                  className="min-h-28"
                  placeholder={t("settings.standingGuidancePlaceholder")}
                />
              </div>
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-800">
                ...
                <Textarea
                  ...
                  maxLength={2000}
                  className="min-h-28"
                  placeholder={t("settings.nextPostFeedbackPlaceholder")}
                />
              </div>
              <Button
                className="w-full"
                ...
                onClick={() =>
                  savePreferences.mutate({
                    editorialGuidance,
                    nextPostToneFeedback: nextPostFeedback,
                  })
                }
              >
                <Save className="h-4 w-4" />
                {t("settings.saveEditorial")}
              </Button>
            </CardContent>
          </Card>
          <Card className="border-indigo-100 bg-indigo-50/50 shadow-sm">
            ...
            <CardContent className="text-sm leading-7 text-indigo-950">
              <p>{t("settings.rulesBody", { signature })}</p>
              <Separator className="my-4 bg-indigo-100" />
              <p>{t("settings.rulesBody2")}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="border-slate-200/80 shadow-sm">
          ...
          <CardContent className="space-y-5">
            ...
            <div className="space-y-2">
              {sources.map(source => (
                <div
                  className="flex flex-col gap-3 rounded-xl border border-slate-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"
                  key={source.id}
                >
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p
                        ...
                        dir="ltr"
                      >
                        {source.feedUrl ?? source.homepage}
                      </p>
                    </div>
                  </div>
